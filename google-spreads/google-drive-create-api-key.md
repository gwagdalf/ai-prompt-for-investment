...
### 4단계: OAuth 동의 화면 설정 (중요!)

> ⚠️ `https://console.cloud.google.com/apis/credentials/consent` 주소로 바로 가면
> `https://console.cloud.google.com/auth/overview` 페이지로 리다이렉트되며
> **"Google 인증 플랫폼이 아직 구성되지 않음"** 메시지가 나옵니다.
>
> **이것은 에러가 아닙니다.** 아직 한 번도 설정을 시작한 적 없어서 나오는 **정상적인 안내 메시지**입니다.
> Google Drive 무료/유료 사용자와 관계없이 **모든 사람에게 동일하게 표시**됩니다.
>
> **해결 방법**: 페이지 중앙이나 하단에 있는 **"시작하기"** 버튼을 클릭하면
> 실제 설정 화면으로 이동합니다. "시작하기"를 누르기 전에는 설정 화면 자체가 존재하지 않습니다.

1. 좌측 메뉴에서 **"API 및 서비스"**를 클릭합니다. 마우스를 올리면 하위 메뉴가 나옵니다.
2. **"OAuth 동의 화면"**을 클릭합니다.
3. **`/auth/overview` 페이지로 이동하며 "Google 인증 플랫폼이 아직 구성되지 않음" 메시지가 표시됩니다.**
4. 페이지 중앙 또는 하단의 **"시작하기"** (또는 "Configure Consent Screen") 버튼을 클릭합니다.
   - 이 버튼을 클릭해야 실제 설정 화면(`/consent`)이 열립니다.
5. **"사용자 유형"** 선택 화면이 나타납니다. **"외부"**를 선택하고 **"만들기"**를 클릭합니다.
   > "외부"란: 내 개인 Gmail 계정을 사용한다는 의미입니다. (유료/무료와 무관)
6. 다음 정보를 입력합니다:
...
7. **"저장하고 계속"**을 세 번 클릭하여 나머지 단계를 건너뜁니다.
8. **"대시보드"**로 돌아옵니다.

### 5단계: 테스트 사용자 추가

1. OAuth 동의 화면 대시보드에서 아래로 스크롤하여 **"테스트 사용자"** 섹션을 찾습니다.
2. **"사용자 추가"** 버튼을 클릭합니다.
